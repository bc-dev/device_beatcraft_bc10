...
nmea_reader_update_date( NmeaReader*  r, Token  date, Token  time )
{
    ...
    if (tok.p + 6 != tok.end) {
        BC10_GPS_DEBUG("date not properly formatted: '%.*s'", tok.end-tok.p, tok.p);
        return -1;
    }
...
nmea_reader_parse( NmeaReader*  r )
{
    ...
    */
    ...
    if (tok.p + 5 > tok.end) {
        BC10_GPS_DEBUG("sentence id '%.*s' too short, ignored.", tok.end-tok.p, tok.p);
        return;
    }
    ...
    if ( !memcmp(tok.p, "GGA", 3) ) {
        ...
        if (tok_fixStatus.p[0] != '\0' && tok_fixStatus.p[0] != '1') {
          ...
          for (i = 3; i <= 14; ++i){
            ...
            if (prn > 0){
              ...
              BC10_GPS_DEBUG("%s: fix mask is %d", __FUNCTION__, r->sv_status.used_in_fix_mask);
            }
        ...
        if (noSatellites > 0) {
          ...
          BC10_GPS_DEBUG("%s: GSV message with total satellites %d", __FUNCTION__, noSatellites);   
...
const void* gps_bc10_get_extention(const char* name)
{
    BC10_GPS_DEBUG("gps_bc10_get_extension called!:%s",name);
...
void gps_bc10_delete_aiding_data(GpsAidingData flags)
{
    BC10_GPS_DEBUG("gps_bc10_delete_aiding_data called!");
...
int gps_bc10_inject_time(GpsUtcTime time, int64_t timeReference, int uncertainty)
{
    BC10_GPS_DEBUG("gps_bc10_inject_time called!");
...
void gps_bc10_set_fix_frequency(int frequency)
{
    BC10_GPS_DEBUG("gps_bc10_set_fix_frequency called!");
...
int gps_bc10_set_position_mode(GpsPositionMode mode, int fix_frequency)
{
    //   This method is called via gps_bc10_start function.
    BC10_GPS_DEBUG("gps_bc10_set_position_mode called!");
    ...
    gps_bc10_set_fix_frequency(fix_frequency);

    return 0;
}

void gps_bc10_cleanup(void)
{
    BC10_GPS_DEBUG("gps_bc10_set_fix_cleanup called!");
...
int gps_bc10_stop(void)
{
    BC10_GPS_DEBUG("gps_bc10_stop called!");
...
static void* gps_bc10_reader_thread(void *args)
{
    BC10_GPS_DEBUG("gps_bc10_reader_thread started!");
    ...
    if (ret == EOF) {
        BC10_GPS_ERROR("gps_bc10_reader_thread: init value set error!(104)");
    } else {
    ...
    if (ret == EOF) {
        BC10_GPS_ERROR("gps_bc10_reader_thread: init value set error!(106)");
    } else {
    ...
    BC10_GPS_DEBUG("gps_bc10_reader_thread ended!");

    return 0;
}

int gps_bc10_start(void)
{
    BC10_GPS_DEBUG("gps_bc10_start called!");
    ...
    ret = pthread_create(
        &gps_state->thread,
        NULL,
        gps_bc10_reader_thread,
        NULL
    );
    if (ret != 0) {    
        BC10_GPS_ERROR("gps_bc10_start failed because of thread creation failure: %d", ret);
        return ret;
    }
...
static int gps_bc10_term_init(int fd)
{
    int ret;
    unsigned int speed;
    ...
    if (ret < 0) {
        BC10_GPS_ERROR("gps_bc10_term_init: serial port attribute get failed!");
        return 1;
    }
    
    speed = cfgetispeed(&ios);
    BC10_GPS_DEBUG("gps_bc10_term_init: got serial port speed %u", speed);

    ret = cfsetispeed(&ios, B57600);
    if (ret < 0) {
        BC10_GPS_ERROR("gps_bc10_term_init: serial port setspeed failed!");
        return 1;
    }

    speed = cfgetispeed(&ios);
    BC10_GPS_DEBUG("gps_bc10_term_init: set serial port speed %u", speed);

    if (ios.c_cflag | CRTSCTS) {
        BC10_GPS_DEBUG("gps_bc10_term_init: hardware flow control is enabled");
        ios.c_cflag &= ~CRTSCTS;
        BC10_GPS_DEBUG("gps_bc10_term_init: disable hardware flow control");
    }

    ret = tcsetattr(fd, TCSANOW, &ios);
    if (ret < 0) {
        BC10_GPS_ERROR("gps_bc10_term_init: serial port attribute set failed!");
        return 1;
    }
    
    BC10_GPS_DEBUG("gps_bc10_term_init: success %s", TTY_DEV);

    return 0;
}
int gps_bc10_init(GpsCallbacks *callbacks)
{
    BC10_GPS_DEBUG("gps_bc10_init called!");
    ...
    int ret = 0;
    if (fd < 0) {
        BC10_GPS_ERROR("gps_bc10_init: gps device open failed! : %s", TTY_DEV);
        return 1;
    }

    BC10_GPS_DEBUG("gps_bc10_init: successfully opened %s", TTY_DEV);

    ret = sem_init(&gps_state->fix_sem , 0, 1);
    if (ret == -1) {
        BC10_GPS_ERROR("gps_bc10_init: gps fix_sem init failed!");
        return 1;
    }

    ret = gps_bc10_term_init(fd);
    if (ret != 0) {
        BC10_GPS_ERROR("gps_bc10_init: gps device init failed!");
        return 1;
    }
    ...
    fp = fdopen(fd, "w+");
    if (!fp) {
        BC10_GPS_ERROR("gps_bc10_init: device file open(fdopen) failed!");
        return 1;
    }
    ...
    BC10_GPS_DEBUG("gps_bc10_init: success");
...
//
GpsInterface bc10_gps = {
    gps_bc10_init,
    gps_bc10_start,
    gps_bc10_stop,
    gps_bc10_set_fix_frequency,
    gps_bc10_cleanup,
    gps_bc10_inject_time,
    gps_bc10_delete_aiding_data,
    gps_bc10_set_position_mode,    
    gps_bc10_get_extention,
};
